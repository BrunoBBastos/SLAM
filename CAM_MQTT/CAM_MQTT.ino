...
const char *mqtt_server = "10.7.220.187";
...
void sendVelocities() {
  char buffer[50];
  snprintf(buffer, sizeof(buffer), "V %f %f\n", VelEsqDir[0], VelEsqDir[1]);
  Serial.println(buffer);
}

void sendPWM() {
  char buffer[50];
  snprintf(buffer, sizeof(buffer), "P %d %d\n", PWMEsqDir[0], PWMEsqDir[1]);
  Serial.println(buffer);
}
...
void handleSerial() {
  if (Serial.available()) {
    ...
    if (cmd == 'O') {
      odometriaDelta[0] += Serial.parseFloat();
      odometriaDelta[1] += Serial.parseFloat();
      odometriaDelta[2] += Serial.parseFloat();
      updateOdom = true;
    }
  }
...
/// Ajusta um ângulo para o intervalo [-pi, pi]
...
/// Retorna a distância entre dois pontos em um plano 2D
/// p1, p2: [x1, y1], [x2, y2]
...
void mqtt_robot_handle_publish(const String &topic) {
  ...
  // p += sprintf(p, "\"Velocidades\":\"%f, %f\",", VelEsqDir[0], VelEsqDir[1]);
  // p += sprintf(p, "\"Modo\":\"%d\"", currentState);
  // *p++ = '}';
  // *p++ = 0;

  // if (mqtt.publish(topic, json_response) == true) {
...
void mqtt_callback(char *topic, unsigned char *payload, unsigned int length) {
  ...
  // Cheque o type do update e receba as variáveis respectivas
  const char *variable = doc["type"];
  if (!strcmp(variable, "ref")) {
    // Recebe uma referência a ser seguida através do controle establizante
    // (x:x, y:y, theta:t)
    reference[0] = (doc["x"]);
    reference[1] = (doc["y"]);
    reference[2] = (doc["t"]);
    reference[2] = angleWrap(reference[2]);
    velReady = false;
    goalReached = false;
  } else if (!strcmp(variable, "vel")) {
    // Recebe vlores de velocidade linear para cada roda
    ...
    // Recebe uma pose para o robô
    // (x:x, y:y, theta:t)
    pose[0] = (doc["x"]);
    pose[1] = (doc["y"]);
    pose[2] = (doc["t"]);
    pose[2] = angleWrap(pose[2]);
  } else if (!strcmp(variable, "pwm")) {
    PWMEsqDir[0] = (doc["l"]);
    PWMEsqDir[1] = (doc["r"]);
    pwmReady = true;
    goalReached = true;
  ...
  for (;;) {
    ...
    if (updateOdom) {
      ...
      *p++ = 0;
      ...
      if (mqtt.publish("robot/odometria", json_response)) {
        odometriaDelta[0] = 0.0f;
        odometriaDelta[1] = 0.0f;
        odometriaDelta[2] = 0.0f;
        updateOdom = false;
      }
    }
  }
