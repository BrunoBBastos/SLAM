  ...
  char*  buf;
    ...
    if(!strcmp(variable, "vel")){ 
      ...
      if(httpd_query_key_value(buf, "v", val_linear, sizeof(val_linear)) == ESP_OK &&
        ...
        Velocities[0] = atof(val_linear);
        Velocities[1] = atof(val_angular);
        velReady = true;
      }
    }
...
static esp_err_t robot_handler(httpd_req_t *req)
{
...
void startCameraServer(int hPort, int sPort){
    ...
    httpd_uri_t robot_uri = {
        .uri       = "/robot",
        .method    = HTTP_GET,
        .handler   = robot_handler,
        .user_ctx  = NULL
    };
    httpd_uri_t slam_uri = {
        .uri       = "/slam",
        .method    = HTTP_GET,
        .handler   = slam_handler,
        .user_ctx  = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        if (critERR.length() > 0) {
            ...
            httpd_register_uri_handler(camera_httpd, &robot_uri);
            httpd_register_uri_handler(camera_httpd, &slam_uri);
